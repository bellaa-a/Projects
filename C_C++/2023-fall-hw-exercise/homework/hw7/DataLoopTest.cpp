...
struct DataLoopTest {
  ...
  static void OtherTests() {
    ...
    *r = *q + *q;
    ...
    DataLoop *p = new DataLoop(1);
    *p += 2; *p += 3;
    DataLoop *t = new DataLoop(4);
    p->splice(*t, 0);

    delete t;

    ASSERT(p->count == 4);
    ASSERT(p->start->data == 4);

    delete p;

    DataLoop *a = new DataLoop(1);
    *a += 2;
    DataLoop *b = new DataLoop(3);
    *b = *a + *a;
    ASSERT(b->count == 4);
    ASSERT(b->start->data == 1);
    ASSERT(b->start->next->data == 2);
    ASSERT(b->start->next->next->data == 1);
    ASSERT(b->start->next->next->next->data == 2);
    ASSERT(b->start->next->next->next->next->data == 1);
    ASSERT(a->count == 2);
    ...
    *b = *a + *b;
    ...
    DataLoop *c = new DataLoop(1);
    *c ^ -5;
    ASSERT(c->start->data == 1);
