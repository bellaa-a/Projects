...
public:
   ...
   */ 
  TDataLoop<T>(const TDataLoop<T> & rhs);
   ...
   */
  TDataLoop<T> & operator+=(const T & num);
   ...
   */
  TDataLoop<T> operator+(const TDataLoop<T> & rhs) const;
   ...
   * \return A reference to the updated DataLoop object
   */
  TDataLoop<T> & operator^(int offset);
   ...
   * \return A reference to the updated DataLoop object 
   */
  TDataLoop<T> & splice(TDataLoop & rhs, size_t pos);
  ...
  friend std::ostream & operator<<(std::ostream & os, const TDataLoop<T> & dl) {
    if (dl.count == 0) {
        ...
        TDataLoop<T>::_Node* cur = dl.start;
