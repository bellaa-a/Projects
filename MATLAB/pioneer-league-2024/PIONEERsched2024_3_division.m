function [ForExcel,SchedInfo,problist,nprobs]=PIONEERsched2024_3_division(xstart)
...
runnum=get_run_num_3_division();

directory = ['runs/PIONEER24_3_Division_' num2str(runnum)];
...
DIV_1 = [OAK DAV NCO RMV];
DIV_2 = [BOI IDF OGD GJJ];
DIV_3 = [GRR MIS GFV BIL];
nteams=max([DIV_1 DIV_2 DIV_3]);
divisions = {DIV_1 DIV_2 DIV_3};
%divisions = {WEST, EAST};
...
if whichconstraints(1,5)==1
    ...
    % each team plays every other team in their division at least 18 times
    ...
    % each team hosts every other team in their division at least six times per season
    for divIndex = 1:length(divisions)
    ...
    % each team plays teams in each other division for 21 games
    for divIndex1 = 1:length(divisions)
        division1 = divisions{divIndex1};
        for divIndex2 = 1:length(divisions)
            if divIndex1 ~= divIndex2
                division2 = divisions{divIndex2};
                for j=division1
                    rin = rin + 1
                    for jj=division2
                        for i=1:nslots
    ...
    % host the other teams in each other division for at least 9 games
    for divIndex1 = 1:length(divisions)
        division1 = divisions{divIndex1};
        for divIndex2 = 1:length(divisions)
            if divIndex1 ~= divIndex2
