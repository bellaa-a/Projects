...
int main( int argc, char **argv ) {
  ...
  if (munmap(arr, file_size) != 0) {
    fprintf(stderr, "Error with munmap");
    exit(1);
  }
...
int quicksort( int64_t *arr, unsigned long start, unsigned long end, unsigned long par_threshold ) {
  ...
  if (left_pid == 0) {
    ...
    // ...do work...
    int ret = quicksort(arr, start, mid, par_threshold);
    if (ret) {
      exit(0);
    } else {
      exit(1);
    ...
    // fork failed
    fprintf(stderr, "Left Child Fork failed\n");
    return 0;
  } 

  // Fork child for right
  right_pid = fork();
  if (right_pid == 0) {
    // executing in the child
    // ...do work...
    int ret = quicksort(arr, mid + 1, end, par_threshold);
    if (ret) {
      exit(0);
    } else {
      exit(1);
    ...
    // fork failed
    fprintf(stderr, "Right Child Fork failed\n");
    waitpid(left_pid, NULL, 0);
    return 0;
  } 

  // parent waiting for left
  int lc, lwstatus;
  lc = waitpid( left_pid, &lwstatus, 0 );
  if (lc < 0) {
    fprintf(stderr, "Failed for left child\n");
    left_success = 0;
  } else {
    if ( !WIFEXITED( lwstatus ) ) {
      left_success = 0;
    } else if ( WEXITSTATUS( lwstatus ) != 0 ) {
      left_success = 0;
    } else {
      left_success = 1;
    }
  ...
  int rc, rwstatus;
  rc = waitpid( right_pid, &rwstatus, 0 );
  // parent waiting for right
  if (rc < 0) {
    fprintf(stderr, "Failed for right child\n");
    right_success = 0;
  } else {
    if ( !WIFEXITED( rwstatus ) ) {
      right_success = 0;
    } else if ( WEXITSTATUS( rwstatus ) != 0 ) {
      right_success = 0;
    } else {
      right_success = 1;
    }
