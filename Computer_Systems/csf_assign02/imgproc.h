...
// Get and reflect slice A across the diagonial to get the top left corner
//
// Parameters:
//   midpoint  - half the length
//   sliceA  -  2D array to the slice A
//   result  - 2D array to store the result
void getTopLeftCorner(int totalLength, int midpoint, uint32_t * data, uint32_t result[midpoint][midpoint]);
