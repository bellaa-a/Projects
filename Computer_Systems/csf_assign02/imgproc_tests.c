...
#define ASSERT_SAME(a, b) assert((a) == (b))
...
void test_getRGBA( TestObjs *objs );
void test_getIntRepresentationOfRGB( TestObjs *objs );
...
int main( int argc, char **argv ) {
  ...
  TEST ( test_getRGBA );
  TEST ( test_getIntRepresentationOfRGB );
...
TestObjs *setup( void ) {
  ...
  objs->sq_test_pic_odd = sq_test_pic_odd;
  objs->sq_test_odd = picture_to_img( &sq_test_pic_odd );
  objs->sq_test_out_odd = (struct Image *) malloc( sizeof( struct Image ) );
  img_init( objs->sq_test_out_odd, objs->sq_test_odd->width, objs->sq_test_odd->height );
  return objs;
}
...
void cleanup( TestObjs *objs ) {
  ...
  destroy_img( objs->sq_test_out_odd );
  destroy_img( objs->sq_test_odd );
...
// Test case for getRGBA
void test_getRGBA( TestObjs *objs ) {
 uint32_t result[4];

  // Test case 1: RGBA(255, 128, 64, 32)
  getRGBA(0xFF804020, result);
  
  ASSERT_SAME(result[0], 0xFF); // Red
  ASSERT_SAME(result[1], 0x80); // Green
  ASSERT_SAME(result[2], 0x40); // Blue
  ASSERT_SAME(result[3], 0x20); // Alpha

  // Test case 2: RGBA(0, 0, 0, 0)
  getRGBA(0x00000000, result);
  ASSERT_SAME(result[0], 0x00);
  ASSERT_SAME(result[1], 0x00);
  ASSERT_SAME(result[2], 0x00);
  ASSERT_SAME(result[3], 0x00);

  // Test case 3: RGBA(255, 255, 255, 255)
  getRGBA(0xFFFFFFFF, result);
  ASSERT_SAME(result[0], 0xFF);
  ASSERT_SAME(result[1], 0xFF);
  ASSERT_SAME(result[2], 0xFF);
  ASSERT_SAME(result[3], 0xFF);

  // Test case 4: RGBA(16, 32, 48, 64)
  getRGBA(0x10203040, result);
  ASSERT_SAME(result[0], 0x10);
  ASSERT_SAME(result[1], 0x20);
  ASSERT_SAME(result[2], 0x30);
  ASSERT_SAME(result[3], 0x40);
  
}

void test_getIntRepresentationOfRGB( TestObjs *objs ) {
    uint32_t result[4];
    // Test case 1: RGBA(255, 128, 64, 32) -> 0xFF804020
    result[0] = 0xFF;
    result[1] = 0x80;
    result[2] = 0x40;
    result[3] = 0x20;
    ASSERT_SAME(getIntRepresentationOfRGB(result), 0xFF804020);

    // Test case 2: RGBA(0, 0, 0, 0) -> 0x00000000
    result[0] = 0x00;
    result[1] = 0x00;
    result[2] = 0x00;
    result[3] = 0x00;
    ASSERT_SAME(getIntRepresentationOfRGB(result), 0x00000000);

    // Test case 3: RGBA(255, 255, 255, 255) -> 0xFFFFFFFF
    result[0] = 0xFF;
    result[1] = 0xFF;
    result[2] = 0xFF;
    result[3] = 0xFF;
    ASSERT_SAME(getIntRepresentationOfRGB(result), 0xFFFFFFFF);

    // Test case 4: RGBA(16, 32, 48, 64) -> 0x10203040
    result[0] = 0x10;
    result[1] = 0x20;
    result[2] = 0x30;
    result[3] = 0x40;
    ASSERT_SAME(getIntRepresentationOfRGB(result), 0x10203040);

    // Test case 5: RGBA(1, 2, 3, 4) -> 0x01020304
    result[0] = 0x01;
    result[1] = 0x02;
    result[2] = 0x03;
    result[3] = 0x04;
    ASSERT_SAME(getIntRepresentationOfRGB(result), 0x01020304);
}
